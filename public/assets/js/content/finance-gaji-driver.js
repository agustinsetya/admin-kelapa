...
$((function () {
    ...
    $("body").on("click", "#btn-proses-gaji-driver", function () {
        ...
        if (selectedData.length > 0) {
            prosesGajiDriver(selectedData, start_date, end_date, buttonId);
        } else {
            confirmProcess(() => {
                ...
                prosesGajiDriver(allData, start_date, end_date, buttonId);
            });
        }
    });    
}));
...
function getAllDataFromGajiTable() {
    ...
    $('.dt-gajiDriverTable tbody tr').each(function () {
        const dataId = $(this).find('input.proses-gaji-driver').data('id');
        if (dataId) {
...
function prosesGajiDriver(dataToSend, start_date, end_date, buttonId) {
    ...
    .done(function (response) {
        ...
        if (response?.success) {
            ...
            applyFilterGajiDriver(_activeFilter.gudang, start_date, end_date);
        } else {
