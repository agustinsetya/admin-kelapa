...
$((function () {
    ...
    $("body").on("click", "#btn-proses-gaji-pacak-mesin", function () {
        ...
        if (selectedData.length > 0) {
            prosesGajiPacakMesin(selectedData, start_date, end_date, buttonId);
        } else {
            confirmProcess(() => {
                ...
                prosesGajiPacakMesin(allData, start_date, end_date, buttonId);
            });
        }
    });    
}));
...
function applyFilterGajiPacakMesin(gudang = null, start = '', end = '') {
    ...
    getDataGajiPacakMesin(_activeFilter.gudang, _activeFilter.start_date, _activeFilter.end_date).done(function(response) {
        ...
        initializeFinanceGajiPacakMesinTable(rows);
...
function getDataGajiPacakMesin(gudang = null, start = '', end = '') {
    return $.ajax({
        url: base_url + 'finance/gaji-pacak-mesin/data',
        method: 'GET',
        data: {
            gudang_id: gudang,
            start_date: start,
...
function initializeFinanceGajiPacakMesinTable(data) {
    const $dgp = $(".dt-gajiPacakMesinTable").first();
    ...
    $dgp.DataTable({
        ...
        columnDefs: [
            ...
            },
            {
                targets: 2,
                render: function(data, type, row, meta) {
                    var namaPacakMesin = data ? data : "-";
                    var gudang = row.nama_gudang ? row.nama_gudang : "-";

                    return `
                        <div class="d-flex flex-column align-items-start">
                            <span>${namaPacakMesin}</span>
                            <span>${gudang}</span>
                        </div>
                    `;
                }
            ...
            },
            {
                ...
                render: function (data, type, row, meta) {
                    return (
                        '<div class="d-flex flex-column align-items-center"><input type="checkbox" class="form-check-input proses-gaji-pacak-mesin" data-id="' +
...
function getAllDataFromGajiTable() {
    ...
    $('.dt-gajiPacakMesinTable tbody tr').each(function () {
        const dataId = $(this).find('input.proses-gaji-pacak-mesin').data('id');
        if (dataId) {
...
function getSelectedDataFromCheckbox() {
    ...
    $(".proses-gaji-pacak-mesin:checked").each(function () {
...
function prosesGajiPacakMesin(dataToSend, start_date, end_date, buttonId) {
    showBtnLoading(buttonId, { text: "Proses Gaji Pacak Mesin..." });

    $.ajax({
        url: base_url + 'finance/gaji-pacak-mesin/add',
        method: 'POST',
        dataType: 'json',
    ...
    .done(function (response) {
        ...
        if (response?.success) {
            successAlert('Proses Gaji Pacak Mesin Berhasil!');
            applyFilterGajiPacakMesin(_activeFilter.gudang, start_date, end_date);
        } else {
