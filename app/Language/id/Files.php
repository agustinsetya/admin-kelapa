<?php

/**
 * Files language strings.
 * Language=> id => Indonesian
 */

return [
    "Nama_CV" => "CV Ecotech",
    "Profile" => "Profil",
    "Lock_Screen" => "Kunci Layar",
    "Signin" => "Masuk",
    "Login" => "Masuk",
    "Logout" => "Keluar",
    "Menu" => "Menu",
    "Dashboard" => "Dashboard",
    "Data_Utama" => "Data Utama",
    "User_Roles" => "Peran Pengguna",
    "Nama_Peran" => "Nama Peran",
    "Lingkup_Peran" => "LIngkup Peran",
    "Pegawai" => "Pegawai",
    "User" => "Pengguna",
    "Gudang" => "Gudang",
    "Nama_Gudang" => "Nama Gudang",
    "Input_Nama_Gudang" => "Masukkan Nama Gudang",
    "Komponen_Gaji" => "Komponen Gaji",
    "Kategori_Pengeluaran" => "Kategori Pengeluaran",
    "Nama_Kategori_Pengeluaran" => "Nama Kategori Pengeluaran",
    "Keterangan" => "Keterangan",
    "Detail_Komponen_Gaji" => "Detail Komponen Gaji",
    "Berat_Kelapa" => "Berat Kelapa",
    "Berat_Daging" => "Berat Daging",
    "Berat_Kopra" => "Berat Kopra",
    "Berat_Kulit" => "Berat Kulit",
    "Berat_Daging_Diterima" => "Berat Daging yang Diterima",
    "Berat_Daging_Ditolak" => "Berat Daging yang Ditolak",
    "Berat_Limbah" => "Berat Limbah Produksi",
    "Takaran_Daging" => "Takaran Daging",
    "Takaran_Kopra" => "Takaran Kopra",
    "Takaran_Kulit" => "Takaran Kulit",
    "Upah_Daging" => "Upah per Takaran Daging",
    "Upah_Kopra" => "Upah per Takaran Kopra",
    "Upah_Kulit" => "Upah per Takaran Kulit",
    "Elements" => "Elemen",
    "Supply_Chain" => "Supply Chain",
    "Data_Pembelian" => "Pembelian",
    "Daging_Kelapa" => "Daging Kelapa",
    "Kulit_Kelapa" => "Kulit Kelapa",
    "Kopra" => "Kopra",
    "Data_Pengolahan" => "Pengolahan",
    "Data_Pengiriman" => "Pengiriman",
    "Data_Penjualan" => "Penjualan Produksi",
    "Data_Penjualan_Limbah" => "Penjualan Limbah Produksi",
    "Data_Pacak_Mesin" => "Pacak Mesin",
    "Pendapatan_Limbah" => "Pendapatan Limbah Produksi",
    "Finance" => "Keuangan",
    "Pengeluaran" => "Pengeluaran",
    "Gaji_Driver" => "Gaji Sopir",
    "Gaji_Driver_Distribusi" => "Gaji Sopir Distribusi",
    "Gaji_Driver_Ngepok_Truk" => "Gaji Sopir Ngepok Truk",
    "Gaji_Driver_Ngepok_Pickup" => "Gaji Sopir Ngepok Pickup",
    "Gaji_Pacak_Mesin" => "Gaji Pacak Mesin",
    "Gaji_Pegawai" => "Gaji Pegawai",
    "Date" => "Tanggal",
    "Report" => "Laporan",
    "Report_Pengolahan" => "Laporan Pengolahan",
    "Machine Operator" => "Laporan Gaji Pacak Mesin",
    "Report_Gaji_Driver" => "Laporan Gaji Sopir",
    "Report_Gaji_Pegawai" => "Laporan Gaji Pegawai",
    "Input_Berat" => "Masukkan Berat",
    "Input_Upah" => "Masukkan Upah",
    "Input_Bonus" => "Masukkan Bonus",
    "Input_Jumlah" => "Masukkan Jumlah",
    "Data_Penggajian" => "Data Penggajian",
    "Login_Invalid" => "Username atau Password Salah",
    "Password_Required" => "Password Diperlukan",
    "Password_Wrong" => "Password Salah",
    "Do_Not_Have_Access" => "Anda Tidak Punya Akses",
    "Username" => "Username",
    "Password" => "Password",
    "Email" => "Email",
    "Input_Username" => "Masukkan Email",
    "Input_Password" => "Masukkan Password",
    "Welcome_Back" => "Selamat Datang Kembali",
    "Login_To_Continue" => "Masuk Untuk Melanjutkan",
    "Reenter_Password" => "Masukkan Kata Sandi Anda untuk Membuka Kunci Layar",
    "Unlock" => "Buka Kunci",
    "Not_You" => "Bukan Anda ?",
    "Created_By" => "PUSTIK ITN",
    "Update" => "Perbarui",
    "Save" => "Simpan",
    "Close" => "Tutup",
    "Laporan" => "Laporan",
    "Biaya" => "Biaya",
    "Input_Biaya" => "Masukkan Biaya",
    "Jumlah" => "Jumlah",
    "Input_Jumlah" => "Masukkan Jumlah",
    "Input_Nama" => "Masukkan Nama",
    "Bonus_Produksi" => "Bonus Produksi",
    "Bonus_Pengiriman" => "Bonus Pengiriman",
    "Bonus_Pacak_Mesin" => "Bonus Pacak Mesin",
    "Jumlah_Perjalanan" => "Jumlah Perjalanan",
    "Jumlah_Kelapa" => "Jumlah Kelapa",
    "Kode_Pegawai" => "Kode Pegawai",
    "Input_Kode_Pegawai" => "Masukkan Kode Pegawai",
    "Kode_Container" => "Kode Container",
    "Input_Kode_Container" => "Masukkan Kode Container",
    "Nama_Pegawai" => "Nama Pegawai",
    "Input_Nama_Pegawai" => "Masukkan Nama Pegawai",
    "Kasbon" => "Pinjaman Pegawai",
    "Detail_Kasbon" => "Detail Pinjaman Pegawai",
];